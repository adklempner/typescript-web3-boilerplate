...
describe('StateMachineBuilder', () => {
  ...
  let account: string;
  let stateMachineBuilderAddress: string;
  before(async () => {
      const accounts = await web3Wrapper.getAvailableAddressesAsync();
      account = accounts[0];

      const stateMachineBuilder = await deployer.deployAsync(ContractName.StateMachineBuilder);
      stateMachineBuilderAddress = stateMachineBuilder.address;
  });
